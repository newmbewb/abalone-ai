...
class MCTSBotCritic(Agent):
    def __init__(self, encoder, critic, selector="exponential", name=None, num_rounds=10, temperature=1.5):
        super().__init__(name)
    ...
    def simulate_random_game(self, game):
        encoded_board = self.encoder.encode_board(game.board, game.next_player)
        value = self.critic.predict(np.array([encoded_board]))[0]
        win_rate = (value + 1) / 2
