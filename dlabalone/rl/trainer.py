...
def _train_on_experience(models, encoder, exp_dir, batch_size, train_fn):
    ...
    for file in os.listdir(exp_dir):
        data_buffer.append_h5file(file)
...
def train_ac(models, encoder, predict_convertor, train_fn, exp_dir, model_directory,
             max_generation=None, num_games=1024, comparison_game_count=512, next_generation_win_rate=0.55,
             move_selectors=None, batch_size=1024, model_name='model'):
    ...
    for _ in range(max_generation):
        ...
        if win_rate > next_generation_win_rate:
            print(f'New model accepted! Generation {generation}, after {episode_count} episodes. {total_games} games')
